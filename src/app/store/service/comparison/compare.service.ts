...
export class CompareService {
  ...
  deleteLocalStorageProduct(id: number) {
    ...
    comparison = JSON.parse(<string>localStorage.getItem(this.comparisonLocalStorage));
    comparison = comparison.filter(product => product.id !== id);
    localStorage.setItem(this.comparisonLocalStorage, JSON.stringify(comparison));
  }
  ...
  addProductToCompareSessionStorage(productData: Product) {
    if (!localStorage.getItem(this.comparisonLocalStorage)) {
      ...
      let comparison: Product [] = JSON.parse(<string>localStorage.getItem(this.comparisonLocalStorage));
      if (comparison.length >= 4) {
        alert("You can`t add more then 4 product");
      }
      else {
  ...
  getAllComparedProductsFromLocalStorage() {
    return JSON.parse(<string>localStorage.getItem(this.comparisonLocalStorage));
  }

  addProductToCompareServer(productId: number) {
    return this.http.post(`${this.comparisonURL}/${productId}`, productId)
      .pipe(
        catchError((error) => {
