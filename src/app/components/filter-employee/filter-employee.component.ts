...
import {
  ROLES_PARAM,
  STATUSES_PARAM,
} from './filter-employee-params.constants';
...
export class FilterEmployeeComponent implements OnInit {
  ...
  getRolesList(): void {
    this.employeeService.getRoles().subscribe((roleFilter: RoleModel[]) => {
  ...
  savedFilters(): void {
    ...
    if (savedRoles != null) this.roleFilter = JSON.parse(savedRoles);
  ...
  changeSelection(): void {
    sessionStorage.setItem(ROLES_PARAM, JSON.stringify(this.roleFilter));
