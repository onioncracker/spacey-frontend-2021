...
@Component({
  selector: 'order-in-history',
  templateUrl: './order-in-history.component.html',
  styleUrls: ['./order-in-history.component.css'],
})
export class OrderInHistoryComponent implements OnInit {
  title = 'Orders';
  order!: OrderDetails;
  status!: string;
  ...
  private findOrderById(): void {
    this.ordersService
      .findOrderByIdForCustomer(this.getOrderId())
      .subscribe((data) => {
        this.order = data;
      });
  }
